...
def show_result(result, *, operation: str, params: dict, input_features: int, input_crs: str | None, elapsed: float):
    ...
    with tab_data:
        if isinstance(nodes_gdf, dict):
            for name, layer in nodes_gdf.items():
                st.write(f"**Nodes — {name}**"); st.dataframe(layer.drop(columns="geometry", errors="ignore"), use_container_width=True)
        ...
        if isinstance(edges_gdf, dict):
            for name, layer in edges_gdf.items():
                st.write(f"**Edges — {name}**"); st.dataframe(layer.drop(columns="geometry", errors="ignore"), use_container_width=True)
    ...
    with tab_record:
        st.json(record); st.download_button("Reproducibility JSON", json_bytes(record), "netgraph_analysis.json", "application/json")
...
if workflow == "Proximity / Contiguity":
    ...
    if gdf is not None:
        ...
        operation = OPERATIONS[operation_key]; params = {}
        ...
        if st.button("🚀 Run City2Graph", type="primary", use_container_width=True):
            ...
            try:
                result = run_operation(gdf, operation_key, **params); elapsed = (datetime.now(timezone.utc) - started).total_seconds()
